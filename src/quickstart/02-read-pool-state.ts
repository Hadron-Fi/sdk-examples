 ...
 *
 * Loads the pool created by script 01 (from output/pool-config.json)
 * and prints all key state: midprice, spread, active curves with
 * decoded points, vault balances, and oracle metadata.
 *
 * Prerequisites:
 *   Run script 01 first on devnet to create the pool:
 *     npm run init
 *
 * Run:
 *   npm run read
...
(async () => {
  ...
  if (process.env.POOL) {
    ...
    if (!fs.existsSync(configPath)) {
      throw new Error(
        "output/pool-config.json not found. Run script 01 on devnet first:\n" +
        "  npm run init\n" +
