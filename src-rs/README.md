...
```
cargo run --bin init-pool       Create pool on devnet (mints, curves, deposit)
      │                            ↳ saves pool address to output/pool-config.json
      ▼
cargo run --bin read-pool       Inspect pool state (midprice, spread, curves, balances)
      │
      ▼
cargo run --bin write-pool      Update midprice, spread, curves, execute swaps
      │
      ▼
cargo run --bin spread-config   Configure spread triggers, swap at different widths
```

Point at a specific pool: `POOL=<address> cargo run --bin read-pool`
...
Uses [`hadron-sdk`](../../hadron-sdk-rs) (Rust SDK) with the `rpc` feature for on-chain reads and writes via `solana-client`.
